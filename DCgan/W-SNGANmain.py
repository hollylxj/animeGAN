...
### load project files
import WSNmodels
from WSNmodels import weights_init

## SNGan functions
# import math
# import chainer
# from chainer import functions as F
# from SNGAN.sn_convolution_2d import SNConvolution2D
...
parser.add_argument('--binary', default=False, help='z from bernoulli distribution, with prob=0.5')
...
arg_list = [
     ...
     '--niter', '200',
     '--lr', '0.0002',
     '--beta1', '0', #dc gan used 0.5
            #'--cuda',
     '--ngpu', '1',
     '--netG', '',
     '--netD', '',
     '--outDir', 'W-SNGANresults',
     '--model', '2',
...
# load models 
n_extra_layers = 0
netG = WSNmodels._netG_1(opt.imageSize, nz, nc, ngf, ngpu, n_extra_layers)
netD = WSNmodels._netD_1(opt.imageSize, nz, nc, ndf, ngpu, n_extra_layers)
...
# setup optimizer
optimizerD = optim.Adam(netD.parameters(), lr = opt.lr, betas = (0, 0.9))
optimizerG = optim.Adam(netG.parameters(), lr = opt.lr, betas = (0, 0.9))
...
#         errD_fake = criterion(output, label)
        ...
        errD_fake.backward(mone) # gradients for fake/real will be accumulated
...
#                  errD.data[0], errG.data[0], D_x, D_G_z1, D_G_z2, end_iter-start_iter))
        print('[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f Elapsed %.2f s'
              % (epoch, opt.niter, i, len(dataloader),
                 errD.data[0], errG.data[0], end_iter-start_iter))
        if i % 100 == 0:
